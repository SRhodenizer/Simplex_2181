...
void Application::Display(void)
{
	...
	//sets up lerp stops 
	static int stop = 0;
	vector3 startPos = m_stopsList[stop];
	vector3 endPos = m_stopsList[(stop + 1) % m_stopsList.size()];
	...
	v3CurrentPos = glm::lerp(startPos, endPos, fPercentage);
	...
	if (fPercentage >= 1.0f)
	{
		stop++; //go to the next route
		fTimer = m_pSystem->GetDeltaTime(uClock);//restart the clock
		stop %= m_stopsList.size();//make sure we are within boundries
	}
